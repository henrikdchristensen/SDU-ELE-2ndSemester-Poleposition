...
;		Bluetooth Setup:
		...
		out		UBRRH,	R16			
		ldi		R16,	12				; UBBR=207 (1 MHz) begrund for 9600 baudrate, 0.2% error
		out		UBRRL,	R16			
		ldi		R16,	0b00000010		; Double USART transmission speed(U2X)
		out		UCSRA,	R16
		ldi		R16,	0b11011000		; enable recieive / Transmit interrupt. enable receiver / enable Transmission
		out		UCSRB,	R16
		ldi		R16,	0b10000110		; 8bit, no parity, 1 stop bit
		out		UCSRC,	R16			

;		A/D konvertering setup:		
		ldi		R16,	0b11001011		; Enable ADC, Enable ADC interrupt, prescaler 8
		out		ADCSRA,	R16				; Load ADCSRA
		ldi		R16,	0b00100001 		; 5V ref, left-justified, PA1
